...
function prob_z_x = sensor_on(sensors, prob_z_x)
    ...
    sensor_prob = sensor_prob' * sensor_prob;
    

    for i = 1: size(sensors,1)
        center = sensors(i);
        prob_z_x(center(1)-2 : center(1)+2, center(2)-2: center(2)+2) = sensor_prob;
    end
    %확률 출력
...
state_transition = state_transition_x * state_transition_y';

state_transition_matrix = zeros(n^2, n^2);

for i = 1: n
    for j = 1:n
        intermidiate_n_n = zeros(n,n);
        point = i + n*j;
        
        intermidiate_n_n(i:i+4, j:j+4) = state_transition;

        

        state_transition_matrix(point,)
    end
