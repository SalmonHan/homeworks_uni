...
function [prob_z_x, prob_z_xy] = sensor_on(sensors, prob_z_x, prob_z_xy)
    ...
    for i = 1: size(sensors,1)
        for j = 1: size(sensor_prob(:),1)
            ...
            prob_z_xy(sensors(i,1)+j-3,sensors(i,2)+j-3) = sensor_prob(j);
        end
    end
    %확률 출력
    prob_z_x(:,1,1) = prob_z_x(:,1,1) / sum(prob_z_x(:,1,1));
    prob_z_x(:,1,2) = prob_z_x(:,1,2) / sum(prob_z_x(:,1,2));
    %prob_z_xy = prob_z_xy/ sum(prob_z_xy(:));
end
...
for t = 1:n
    ...
    for i = 1:size(sensors,1)
        if norm(x-sensors(i,:),2) <= 5
            [prob_z_x, prob_z_xy] = sensor_on(sensors, prob_z_x, prob_z_xy);
    ...
    if(x(1)>5)
        prob_z_xy(x(1)-2:x(1)+2,x(2)-2:x(2)+2)
    end    
    ...
    for i = 1:n
        for j = 1:n
            belief_xy(j,i) = prob_z_xy(i,j) * belief(i,1,1) * belief(j,1,2);
        end
    end
    ...
    % belief_xy에서 x,y롤 마지널리티 계산
    belief_x = sum(belief_xy, 1);
    belief_y = sum(belief_xy, 2);
