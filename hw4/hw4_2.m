...
function samples = create_collision_free_samples(n, position)
    range_x = [-1, 1];
    range_y = [-1, 1];
    range_z = [-1, 1];
    ...
    num_samples = n;
    samples_x = (range_x(2) - range_x(1)) * rand(num_samples, 1) + range_x(1) + position(1);
    samples_x = min(3, samples_x);
    samples_x = max(-3, samples_x);
    samples_y = (range_y(2) - range_y(1)) * rand(num_samples, 1) + range_y(1) + position(2);
    samples_y = min(3, samples_y);
    samples_y = max(-3, samples_y);
    samples_z = (range_z(2) - range_z(1)) * rand(num_samples, 1) + range_z(1) + position(3);
    samples_z = min(3, samples_z);
    samples_z = max(-3, samples_z);
    samples_psi = (range_psi(2) - range_psi(1)) * rand(num_samples, 1) + range_psi(1) + position(4);
    samples_psi = min(samples_psi, 360-samples_psi);
    samples_theta = (range_theta(2) - range_theta(1)) * rand(num_samples, 1) + range_theta(1) + position(5);
    samples_theta = min(samples_theta, 360-samples_theta);
    samples_phi = (range_phi(2) - range_phi(1)) * rand(num_samples, 1) + range_phi(1) + position(6);
...
dist_min = inf;
% 1. 첫 노드에서 샘플을 20개 뿌린 뒤 free한곳에 랜덤으로 이동
% q : tree에 이미 포함된 노드
% 2. 모든 q에서 20개씩 샘플 뿌림. free한곳에 가장 많이 떨어진 q를 선택 -> q_now
% 3. q_now에서 뿌린 샘플 중 이동 가능한곳을 q_now의 자식으로 추가
% 2~3을 반복
...
while true
    ...
    for i = 1:size(q_list,2)
        ...
        for j  = 1:size(q_list,2)
            p1 = tree_node_list(q_now,1:6);
            p2 = tree_node_list(q_list(j),1:6);
            if calc_distance(p1,p2) <= 2.0
                w_q = w_q + 1;
            end
        end
        w_q = 1/(1+w_q);
    ...
    num_samples = 5;
    samples = create_collision_free_samples(num_samples, tree_node_list(q_selected, :));
    possible_samples = [];
    ...
    for i = 1:num_samples
        ...
        if isColliding == false
            possible_samples(end + 1, :) = samples(i, :);
            %display(samples);
        end

    end

    for j = 1:1%(min(3, size(possible_samples,1)))
        if isempty(possible_samples)
            break
        end
        tree_node_list(end +1, :) = possible_samples(j, :);
        tree_start = addedge(tree_start, q_selected, size(tree_node_list,1));
        %추가했다면 추가한 샘플들 중에 goal과 충분히 가깝다면 goal과 직빵 연결 가능한애 있는지 check
        
        dist_min = min(dist_min, calc_distance(possible_samples(j, :), goal(:)));
        if calc_distance(possible_samples(j, :), goal(:)) < 3

            devide = 8;
            isColliding = false;
            for k  = 1:devide-1
                point = (k/devide)*possible_samples(j) + (1-(k/devide))*goal;
                isColliding = isColliding | check_collision(rod, point, walls);
            ...
            if isColliding == false
                ...
                %last_interpolation = round(calc_distance(possible_samples(j, :),goal(:))*2);
                %last_interpolation
                %for l = 1:last_interpolation-1
                %    point = (l/last_interpolation)*possible_samples(j,:) + (1-(l/last_interpolation))*goal;
...
fileID = fopen('path_output_hw_4_2.txt', 'w');
